...
flowchart TB
    ...
    subgraph ESP["Especialistas · Cloud Run · ingress interno · AgentCard em /.well-known/agent-card.json"]
        ...
        XLS["Excel Agent<br/>workbooks .xlsx,<br/>URL GCS assinada"]
    end
...
| **Excel** | Cloud Run (A2A, ingress interno) | Entregáveis offline: workbooks .xlsx formatados a partir de queries validados ou dashboards existentes; entrega por URL GCS assinada | `run_inline_query`/`run_query` + openpyxl, `export_query_to_excel`, `export_multi_sheet_excel`, `export_dashboard_to_excel` |
...
├── excel_agent/       # workbooks .xlsx personalizados (openpyxl + GCS assinado)
...
| `EXPORT_BUCKET` / `EXCEL_URL_EXPIRY_HOURS` | excel | Bucket de exports e expiração da URL assinada (padrão 24 h; limpeza automática em 7 dias) |
...
5. O dashboard fica na pasta de destino do Looker: nativo, editável e compartilhável. Um «e me mande em Excel» posterior delega ao **Excel Agent**, que exporta uma folha por tile e devolve a URL assinada de download.
